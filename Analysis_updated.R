...
deg_ctry <- degree(g_ip_cp, v = V(g_ip_cp)[type == FALSE])
deg_ctry

deg_ctry_sorted <- sort(deg_ctry)

top10 <- head(deg_ctry_sorted, 10)
bottom10 <- tail(deg_ctry_sorted, 10)

deg_subset <- c(top10, bottom10)

#Plotting
png("Country_degree_sorted.png", width = 1500, height = 800)

par(mar = c(5, 20, 5, 2))

barplot(deg_subset,
        horiz = TRUE,
        names.arg = names(deg_subset),
        main = "Degree of Country Nodes",
        xlab = "Degree",
        ylab = "",
        las = 1,
        xlim = c(0, max(deg_subset) + 5),
        cex.names = 1.2,    # increase size of category labels (country names)
        cex.axis = 1.2,     # increase tick label size (numbers)
        cex.lab  = 1.4,      # increase x-axis label size)
        space = 1.5 
       )
...
plot(deg_ctry, V(g_ip_cp)[type == FALSE]$eci,
     xlab = "Degree of country",
     ylab = "ECI of implementing countries",
     #cex.names = 1.2,    # increase size of category labels (policy names)
     cex.axis = 1.8,     # increase tick label size (numbers)
     cex.lab  = 1.8,      # increase x-axis label size)
     pch = 19)
...
# Add regression line
abline(lm(V(g_ip_cp)[type == FALSE]$eci~deg_ctry), col = "red", lwd = 2)
r <- cor(deg_ctry, V(g_ip_cp)[type == FALSE]$eci)
  legend("topright", legend = paste0("r = ", round(r, 2)), bty = "n",cex = 2)
...
barplot(deg_p_sorted,
        ...
        xlim = c(0, max(deg_p_sorted) + 5),
        cex.names = 1.2,    # increase size of category labels (policy names)
        cex.axis = 1.3,     # increase tick label size (numbers)
        cex.lab  = 1.4,      # increase x-axis label size)
        space = 1.5,
        width = 2
        )
...
#Product Histogram
deg_product <- degree(g_ip_pp, v = V(g_ip_pp)[type == FALSE])
deg_product

deg_product_sorted <- sort(deg_product)

#Plotting
png("policy_degree_sorted_Product Product.png", width = 1500, height = 800)

par(mar = c(5, 20, 5, 2))

barplot(deg_product_sorted,
        horiz = TRUE,
        names.arg = names(deg_product_sorted),
        main = "Degree of Product Nodes",
        xlab = "Degree",
        ylab = "",
        las = 1,
        xlim = c(0, max(deg_product_sorted) + 5),
        cex.names = 1.2,    # increase size of category labels (policy names)
        cex.axis = 1.3,     # increase tick label size (numbers)
        cex.lab  = 1.4,      # increase x-axis label size)
        space = 1.5
...
par(mfrow = c(3, 2), mar = c(5, 6, 4, 2))
i <- 1
for(t in names(deg1)) {
  hist(deg1[[t]], main=paste("IP - ",t), xlab="degree",cex.lab  = 1.5,cex.axis = 1.2)
  hist(deg2[[t]], main=paste("Non IP - ",t), xlab="degree",cex.lab  = 1.5,cex.axis = 1.2)
